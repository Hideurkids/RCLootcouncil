...
function RCSessionFrame.SetCellDeleteBtn(rowFrame, frame, data, cols, row, realrow, column, fShow, table, ...)
	...
	-- itself; a literal ".png"/".tga" suffix can fail to load on this client (shows as a
	-- missing-texture placeholder, confirmed live to look like this for the icon below).
...
function RCSessionFrame.SetCellItemIcon(rowFrame, frame, data, cols, row, realrow, column, fShow, table, ...)
	-- No file extension on a Blizzard texture path (see SetCellDeleteBtn's comment above).
	local texture = data[realrow].texture or "Interface\\ICONS\\INV_Sigil_Thorim"
...
function RCSessionFrame:GetFrame()
	...
	-- ChatConfigCheckButtonTemplate doesn't exist on this client (confirmed live -
	-- "CreateFrame(): Couldn't find inherited node") - UICheckButtonTemplate is confirmed-safe
	-- (a genuine vanilla template) but has no built-in label region, so add our own FontString.
	local tgl = CreateFrame("CheckButton", f:GetName().."Toggle", f.content, "UICheckButtonTemplate")
	local tglText = tgl:CreateFontString(nil, "OVERLAY", "GameFontNormal")
	tglText:SetPoint("LEFT", tgl, "RIGHT", 2, 0)
