...
-- DIAGNOSTIC: pcall-wrapped so a failure here (e.g. a template that didn't register) can't
-- silently kill every function definition below it in the file (same bug class as core.lua's
-- InterfaceOptionsFrameCancel crash - see CLAUDE.md) - and so the real error is visible instead
-- of manifesting only as "Lib_UIDropDownMenu_Initialize is nil" somewhere else entirely.
local ok, ddErr = pcall(function()
	for i = 1, LIB_UIDROPDOWNMENU_MAXLEVELS do
		...
		local normalTextRegion = _G["Lib_DropDownList1Button1NormalText"];
		local fontHeight = 10;
		if normalTextRegion then
			local fontName, height, fontFlags = normalTextRegion:GetFont();
			fontHeight = height or fontHeight;
		end
...
-- Dropdown menus (Filter/Enchanters/right-click-menu) are deprioritized per round 23's decision -
-- every call site that would use them is already guarded, so a failure here is silent/harmless in
-- normal play. Still report it if it ever happens, since it's the one signal that could help
-- someone eventually root-cause the underlying mystery.
if not ok and DEFAULT_CHAT_FRAME then
	DEFAULT_CHAT_FRAME:AddMessage("RCLootCouncil: UIDropDownMenu.lua failed to initialize: "..tostring(ddErr))
end

function Lib_UIDropDownMenuDelegate_OnAttributeChanged ()
	local self, attribute, value = this, arg1, arg2
...
-- SetNormalFontObject/SetDisabledFontObject/SetHighlightFontObject are later-client-only Button
-- methods that don't exist on this client (confirmed live - see LibDialog-1.0.lua's own fix for
-- the same missing API). Lib_UIDropDownMenuButtonTemplate only has ONE text region ($parentNormalText)
-- shared across all 3 visual states anyway, so falling back to FontString:SetFontObject() (a
-- definitely-safe, ancient API) on that region directly gets the same practical effect - text
-- won't swap to a different font per hover/disabled state, but every dropdown menu addon-wide
-- (votingFrame/lootHistory's right-click filters) stops crashing on the very first button render.
local function Lib_SetButtonFontObject(button, region, method, fontObj)
	if button[method] then
		button[method](button, fontObj)
	else
		region:SetFontObject(fontObj)
	end
end
...
function Lib_UIDropDownMenu_AddButton(info, level)
	...
	-- Default settings
	Lib_SetButtonFontObject(button, normalText, "SetDisabledFontObject", GameFontDisableSmallLeft);
	...
	if ( info.notClickable ) then
		info.disabled = 1;
		Lib_SetButtonFontObject(button, normalText, "SetDisabledFontObject", GameFontHighlightSmallLeft);
	end

	-- Set the text color and disable it if its a title
	if ( info.isTitle ) then
		info.disabled = 1;
		Lib_SetButtonFontObject(button, normalText, "SetDisabledFontObject", GameFontNormalSmallLeft);
	end
	...
	if ( info.text ) then
		...
		if ( info.fontObject ) then
			Lib_SetButtonFontObject(button, normalText, "SetNormalFontObject", info.fontObject);
			Lib_SetButtonFontObject(button, normalText, "SetHighlightFontObject", info.fontObject);
		else
			Lib_SetButtonFontObject(button, normalText, "SetNormalFontObject", GameFontHighlightSmallLeft);
			Lib_SetButtonFontObject(button, normalText, "SetHighlightFontObject", GameFontHighlightSmallLeft);
		end
	else
		button:SetText("");
		icon:Hide();
...
function Lib_ToggleDropDownMenu(level, value, dropDownFrame, anchorName, xOffset, yOffset, menuList, button, autoHideDelay)
	...
	if ( listFrame:IsShown() and (LIB_UIDROPDOWNMENU_OPEN_MENU == tempFrame) ) then
		...
		if ( level == 1 ) then	
			...
			if string.find(bname, "^Lib_DropDownList%d+$") then
				anchorFrame = button;
			else
		...
		if ( level == 1 ) then
			...
			listFrame:ClearAllPoints();
			-- strmatch/string.match doesn't exist on this client (Lua 5.0 predates it) - use
			-- string.find with a capture instead.
			local _, _, parentLevelStr = string.find(anchorFrame:GetName(), "Lib_DropDownList(%d+)");
			listFrame.parentLevel = tonumber(parentLevelStr);
...
if DEFAULT_CHAT_FRAME then DEFAULT_CHAT_FRAME:AddMessage("RCLC DEBUG: UIDropDownMenu.lua reached its own end - Initialize="..tostring(Lib_UIDropDownMenu_Initialize).." CreateInfo="..tostring(Lib_UIDropDownMenu_CreateInfo)) end
