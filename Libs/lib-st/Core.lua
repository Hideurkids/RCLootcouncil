...
do 
	...
	local FireUserEvent = function (self, frame, event, handler, ...)
		-- Confirmed live: handler can be nil here in at least one circumstance not yet pinned
		-- down via static tracing (a stale/partially-registered column callback, most likely) -
		-- guard defensively rather than crash, consistent with this addon's own DefaultEvents
		-- fallback path for when there's simply no user handler for this event.
		if not handler then
			if self.DefaultEvents[event] then
				self.DefaultEvents[event]( unpack(arg, 1, arg.n));
			end
			return
		end
		if not handler( unpack(arg, 1, arg.n)) then
			if self.DefaultEvents[event] then
				self.DefaultEvents[event]( unpack(arg, 1, arg.n));
			end
		end
	...
	local function RegisterEvents (self, events, fRemoveOldEvents) 
		...
		for i, row in ipairs(self.rows) do 
			for j, col in ipairs(row.cols) do
				...
				for event, handler in pairs(events) do
					col:SetScript(event, function()
						if table.offset and type(i) == "number" then
							local realindex = table.filtered[i+table.offset];
							-- This closure has no ... in its parameter list (correct - the engine
						-- gives SetScript handlers zero real args on this client, not a vararg
						-- table), so it never gets its own implicit `arg` - read the real arg1..4
						-- globals directly instead of trying to unpack a nonexistent local `arg`.
						table:FireUserEvent(col, event, handler, row, this, table.data, table.cols, i, realindex, j, table, arg1, arg2, arg3, arg4 );
						end
					end);
				end
			end
		...
		for j, col in ipairs(self.head.cols) do
			...
			for event, handler in pairs(events) do
				col:SetScript(event, function()
					-- No ... in this closure's params (correct on this client) - read the real
					-- arg1..4 globals directly instead of a nonexistent local `arg`.
					table:FireUserEvent(col, event, handler, self.head, this, table.data, table.cols, nil, nil, j, table, arg1, arg2, arg3, arg4);
				end);
			end
		end
		self.events = events;
	...
	local function SetDisplayRows (self, num, rowHeight)
			...
			for j = 1, tgetn(self.cols) do
				...
				if not col then 
					...
					if self.events then
						for event, handler in pairs(self.events) do
							col:SetScript(event, function()
								-- No ... in this closure's params (correct on this client) - read
								-- the real arg1..4 globals directly instead of a nonexistent
								-- local `arg`.
								if table.offset and type(i) == "number" then
									local realindex = table.filtered[i+table.offset];
									table:FireUserEvent(col, event, handler, row, this, table.data, table.cols, i, realindex, j, table, arg1, arg2, arg3, arg4 );
		...
		for i = num + 1, tgetn(self.rows) do
			self.rows[i]:Hide();
		end
		...
		for i = 1, tgetn(cols) do
			...
			if not col then 
				...
				if self.events then
					for event, handler in pairs(self.events) do
						col:SetScript(event, function()
							-- No ... in this closure's params (correct on this client) - read the
							-- real arg1..4 globals directly instead of a nonexistent local `arg`.
							table:FireUserEvent(col, event, handler, row, this, table.data, table.cols, nil, nil, i, table, arg1, arg2, arg3, arg4);
						end);
					end
				end
			end
	...
	local function SortData (self)
		-- sanity check
		if not(self.sorttable) or (tgetn(self.sorttable) ~= tgetn(self.data))then
			self.sorttable = {};
		end
		if tgetn(self.sorttable) ~= tgetn(self.data) then
			for i = 1, tgetn(self.data) do
				self.sorttable[i] = i;
			end
		end 
		...
		while i <= tgetn(self.cols) and not sortby do
	...
	function ScrollingTable:CreateST(cols, numRows, rowHeight, highlight, parent)
			...
			["OnClick"] = function(rowFrame, cellFrame, data, cols, row, realrow, column, table, button, ...)		-- LS: added "button" argument
				if button == "LeftButton" then	-- LS: only handle on LeftButton click (right passes thru)
					if not (row or realrow) then
						...
						elseif cols[column].sort and string.lower(cols[column].sort) == "asc" then
							sortorder = "dsc";
						end
		...
		st.Refresh = function(self)	
			FauxScrollFrame_Update(scrollframe, tgetn(st.filtered), st.displayRows, st.rowHeight);
		...
		scrollframe:SetScript("OnVerticalScroll", function()
			-- pcall-wrapped: confirmed live (votingFrame.lua's own explicit call to this same
			-- Blizzard function crashed deep in UIPanelTemplates.lua) - this scrollframe doesn't
			-- have a real .scrollChild set up the way Blizzard's own FauxScrollFrameTemplate usage
			-- normally expects (rows are drawn manually instead), so this native helper may not be
			-- safe to call directly here. Fall back to a plain Refresh() so scrolling still works
			-- functionally even if this specific helper call fails.
			local ok = pcall(FauxScrollFrame_OnVerticalScroll, this, arg1, st.rowHeight, function() st:Refresh() end);					-- LS: putting st:Refresh() in a function call passes the st as the 1st arg which lets you reference the st if you decide to hook the refresh
			if not ok then st:Refresh() end
		end);
