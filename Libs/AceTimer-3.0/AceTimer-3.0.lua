...
-- This client's 1.12 API has no Animation system at all (CreateAnimationGroup/CreateAnimation
-- are WotLK 3.1+) - timers are driven by a plain per-timer OnUpdate elapsed-accumulator instead
-- of the retail AnimationGroup trick. Hiding the frame pauses OnUpdate firing entirely, which
-- stands in for the old anim:Stop()/anim:Play().
local function OnUpdate()
	local self = this
	self.remaining = self.remaining - arg1
	...
	-- and has been used to create a new timer during the callback.
	if id == self.id then
...
local function new(self, loop, func, delay, ...)
	...
	if timer then
		...
		timer = CreateFrame("Frame", nil, AceTimer.frame)
		timer:Hide()
		timer:SetScript("OnUpdate", OnUpdate)
	end

	-- Very low delays cause erratic firing on this client's OnUpdate resolution.
...
function AceTimer:ScheduleTimer(func, delay, ...)
	...
	return new(self, nil, func, delay, unpack(arg, 1, arg.n))
end
...
function AceTimer:ScheduleRepeatingTimer(func, delay, ...)
	...
	return new(self, true, func, delay, unpack(arg, 1, arg.n))
end
...
-- upgrade existing timers to the latest OnUpdate
