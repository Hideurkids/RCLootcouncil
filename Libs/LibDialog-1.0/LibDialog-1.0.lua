...
-- Lua 5.0: no `#` operator, use table.getn(t); no Blizzard wipe()/table.wipe global either.
...
-- Library namespace.
-----------------------------------------------------------------------
-- Renamed major (was plain "LibDialog-1.0") to avoid colliding with another installed addon's
-- own vendored copy via LibStub's shared registry - "LibDialog-1.0" is a very commonly-vendored
-- library name, and LibStub only ever exposes ONE instance per major name to every asker,
-- regardless of which addon's copy actually "wins" the registration race.
local LibStub = _G.LibStub
local MAJOR = "RCLootCouncil-LibDialog-1.0"
...
local function _ProcessQueue()
    if tgetn(active_dialogs) == MAX_DIALOGS then
        return
    end
...
local function _RefreshDialogAnchors()
    for index = 1, tgetn(active_dialogs) do
        local current_dialog = active_dialogs[index]
        current_dialog:ClearAllPoints()

        if index == 1 then
            local default_dialog = _G.StaticPopup_DisplayedFrames[tgetn(_G.StaticPopup_DisplayedFrames)]
...
local function _ReleaseDialog(dialog)
    ...
    if dialog.editboxes then
        for index = 1, tgetn(dialog.editboxes) do
    ...
    if dialog.checkboxes then
        for index = 1, tgetn(dialog.checkboxes) do
    ...
    if dialog.buttons then
        for index = 1, tgetn(dialog.buttons) do
...
local function _Dialog_OnShow(overrideDialog)
    ...
    -- Confirmed live: OnShow can fire on a dialog frame before dialog.delegate is assigned (a
    -- timing gap in the acquire/build/show sequence, not yet fully traced) - guard rather than
    -- crash; the dialog's own button click handlers capture their own delegate reference
    -- independently and aren't affected by this.
    if delegate and delegate.on_show then
        delegate.on_show(dialog, dialog.data)
    end
end

local function _Dialog_OnHide()
    local dialog = this
    local delegate = dialog.delegate
    ...
    if tgetn(delegate_queue) > 0 then
        local delegate
        repeat
...
-- `this`/`arg1` (elapsed time) as globals instead. A parameter list here would shadow them with nils.
local function _Dialog_OnUpdate()
    local dialog, elapsed = this, arg1
    local delegate = dialog.delegate
...
local function _Dialog_OnEvent()
    local self, event = this, event
    if self[event] then
        return self[event](self, event, arg1, arg2, arg3, arg4, arg5)
    end
end

-- Guard: not every Blizzard StaticPopup_* function this lib wants to hook exists on 1.12
-- (some are later-client StaticPopup refactors) - hooksecurefunc errors if the target isn't
-- a real function, so check first per CLAUDE.md's detect-and-fallback convention.
if not lib.hooked_onhide and type(_G.StaticPopup_OnHide) == "function" then
...
if not lib.hooked_set_up_position and type(_G.StaticPopup_SetUpPosition) == "function" then
...
if not lib.hooked_escape_pressed and type(_G.StaticPopup_EscapePressed) == "function" then
    local dialogs_to_release = {}

    _G.hooksecurefunc("StaticPopup_EscapePressed", function()
        wipe(dialogs_to_release)
...
local function CheckBox_OnClick()
    local checkbox, mouse_button, down = this, arg1, arg2
    local dialog = checkbox:GetParent():GetParent()
...
local function _AcquireCheckBox(parent, index)
    ...
    if not checkbox then
        local container = _G.CreateFrame("Frame", string.format("%s_CheckBoxContainer%d", MAJOR, tgetn(active_checkboxes) + 1), _G.UIParent)
        container:SetHeight(DEFAULT_CHECKBOX_SIZE)

        checkbox = _G.CreateFrame("CheckButton", string.format("%s_CheckBox%d", MAJOR, tgetn(active_checkboxes) + 1), container, "UICheckButtonTemplate")
        checkbox:SetScript("OnClick", CheckBox_OnClick)

        checkbox.container = container
    end
    table.insert(active_checkboxes, checkbox)
...
local function EditBox_OnTextChanged()
    local editbox, user_input = this, arg1
...
local function _AcquireEditBox(dialog, index)
    ...
    if not editbox then
        local editbox_name = string.format("%s_EditBox%d", MAJOR, tgetn(active_editboxes) + 1)

        -- CONFIRMED LIVE: "AutoCompleteEditBoxTemplate" doesn't exist on this client
        -- ("CreateFrame(): Couldn't find inherited node") - flagged as unconfirmed since the
        -- very first survey of this port. The border textures/label built manually just below
        -- already provide all the visual styling regardless of template, so a bare EditBox (no
        -- player-name autocomplete, which this addon's note/text dialogs don't need anyway)
        -- works just as well - pcall so a client where the template DOES exist still gets it.
        local ok, eb = pcall(_G.CreateFrame, "EditBox", editbox_name, _G.UIParent, "AutoCompleteEditBoxTemplate")
        editbox = ok and eb or _G.CreateFrame("EditBox", editbox_name, _G.UIParent)
        editbox:SetWidth(130)
        editbox:SetHeight(32)
        editbox:SetFontObject("ChatFontNormal")

        local left = editbox:CreateTexture(string.format("%sLeft", editbox_name), "BACKGROUND")
        left:SetTexture([[Interface\ChatFrame\UI-ChatInputBorder-Left2]])
        left:SetWidth(32)
        left:SetHeight(32)
        left:SetPoint("LEFT", -10, 0)

        local right = editbox:CreateTexture(string.format("%sRight", editbox_name), "BACKGROUND")
        right:SetTexture([[Interface\ChatFrame\UI-ChatInputBorder-Right2]])
        right:SetWidth(32)
        right:SetHeight(32)
        right:SetPoint("RIGHT", 10, 0)

        local mid = editbox:CreateTexture(string.format("%sMid", editbox_name), "BACKGROUND")
        ...
        local label = editbox:CreateFontString(string.format("%sLabel", editbox_name), "ARTWORK", "GameFontNormalSmall")
    ...
    table.insert(active_editboxes, editbox)
...
local function Button_OnClick()
    local button, mouse_button, down = this, arg1, arg2
...
local function _AcquireButton(parent, index)
    ...
    if not button then
        local button_name = string.format("%s_Button%d", MAJOR, tgetn(active_buttons) + 1)
        ...
        -- SetNormalFontObject/SetDisabledFontObject/SetHighlightFontObject don't exist on this
        -- client's Button widget (a later-client convenience API, same category as the
        -- already-known-missing SetEnabled) - fall back to a single manually-created FontString
        -- registered via SetFontString, same pattern Modules\uiSkin.lua's CreateButton uses.
...
local function _BuildDialog(delegate, data)
    if tgetn(active_dialogs) == MAX_DIALOGS then
        if not queued_delegates[delegate] then
            table.insert(delegate_queue, delegate)
    ...
    if not dialog then
        dialog = _G.setmetatable(_G.CreateFrame("Frame", string.format("%s_Dialog%d", MAJOR, tgetn(active_dialogs) + 1), _G.UIParent), dialog_meta)
        dialog.is_new = true
    ...
    dialog.text:SetJustifyH(delegate.text_justify_h and TEXT_HORIZONTAL_JUSTIFICATIONS[string.upper(delegate.text_justify_h)] or "CENTER")
    dialog.text:SetJustifyV(delegate.text_justify_v and TEXT_VERTICAL_JUSTIFICATIONS[string.upper(delegate.text_justify_v)] or "MIDDLE")
    ...
    if _G.type(delegate.icon) == "string" then
        if not dialog.icon then
            dialog.icon = dialog:CreateTexture(string.format("%sIcon", dialog:GetName()), "ARTWORK")
    ...
    if delegate.buttons and tgetn(delegate.buttons) > 0 then
        dialog.buttons = {}
        ...
        local NUM_BUTTONS = tgetn(dialog.buttons)
    ...
    if delegate.editboxes and tgetn(delegate.editboxes) > 0 then
        dialog.editboxes = {}

        for index = 1, tgetn(delegate.editboxes) do
            table.insert(dialog.editboxes, _AcquireEditBox(dialog, index))
        end
        NUM_EDITBOXES = tgetn(dialog.editboxes)
    ...
    if delegate.checkboxes and tgetn(delegate.checkboxes) > 0 then
        dialog.checkboxes = {}

        for index = 1, tgetn(delegate.checkboxes) do
...
function lib:Register(delegate_name, delegate)
    if _G.type(delegate_name) ~= "string" or delegate_name == "" then
        error(string.format(METHOD_USAGE_FORMAT, "Register", "delegate_name must be a non-empty string"), 2)
    end

    if _G.type(delegate) ~= "table" then
        error(string.format(METHOD_USAGE_FORMAT, "Register", "delegate must be a table"), 2)
    end
...
local function _FindDelegate(method_name, reference)
    ...
    if reference == "" or (reference_type ~= "string" and reference_type ~= "table") then
        error(string.format(METHOD_USAGE_FORMAT, method_name, "reference must be a delegate table or a non-empty string"), 3)
    end
    local delegate

    if reference_type == "string" then
        if not delegates[reference] then
            error(string.format(METHOD_USAGE_FORMAT, method_name, string.format("\"%s\" does not match a registered delegate", reference)), 3)
        end
...
function lib:Spawn(reference, data)
    ...
    if cancel_list then
        for index = 1, tgetn(cancel_list) do
            ...
            if delegate_to_cancel then
                ...
                error(string.format("\"%s\" does not match a registered delegate - unable to cancel", delegate_name), 2)
            end
        end
    end
    ...
    if tgetn(active_dialogs) > 0 then
        dialog:SetPoint("TOP", active_dialogs[tgetn(active_dialogs)], "BOTTOM", 0, 0)
    else
        local default_dialog = _G.StaticPopup_DisplayedFrames[tgetn(_G.StaticPopup_DisplayedFrames)]
    ...
    table.insert(active_dialogs, dialog)
    dialog:Show()
...
function dialog_prototype:Resize()
    ...
    if self.buttons and tgetn(self.buttons) > 0 then
        height = height + 8 + DEFAULT_BUTTON_HEIGHT

        if tgetn(self.buttons) == MAX_BUTTONS then
            width = 440
        end
    end

    if self.editboxes and tgetn(self.editboxes) > 0 then
        local dialog_left = self:GetLeft()
        local dialog_right = self:GetRight()
        local label_offset = 0
        local editbox_offset = 0

        for index = 1, tgetn(self.editboxes) do
    ...
    if self.checkboxes then
        height = height + (DEFAULT_CHECKBOX_SIZE * tgetn(self.checkboxes))
    end
