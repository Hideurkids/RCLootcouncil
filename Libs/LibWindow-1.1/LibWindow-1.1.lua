...
local _, _, MINOR_STR = string.find("$Revision: 8 $", "(%d+)") -- Lua 5.0: no string.match/:match(), use string.find with captures
...
-- This client's OnUpdate scripts get zero real function arguments - the engine sets `this`
-- as a global instead. A parameter list here would shadow it with nil.
lib.utilFrame:SetScript("OnUpdate", function()
	this:Hide()
...
-- This client's OnDragStart/OnDragStop/OnMouseWheel scripts get zero real function arguments -
-- the engine sets `this` (and `arg1` for OnMouseWheel's delta) as globals instead.
local function onDragStart() return lib.OnDragStart(this) end  -- upgradable
local function onDragStop() return lib.OnDragStop(this) end  -- upgradable
...
local function onMouseWheel() return lib.OnMouseWheel(this, arg1) end  -- upgradable
...
lib.utilFrame:SetScript("OnEvent", function()
	local event, key, state = event, arg1, arg2
